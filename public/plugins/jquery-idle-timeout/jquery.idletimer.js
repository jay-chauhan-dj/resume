...
(function ($) {
	...
	$.fn.idleTimeout = function (userRuntimeConfig) {
		...
		openWarningDialog = function () {
			...
			$('body').append('<div class="modal fade" id="idle-timeout-dialog" data-backdrop="static"><div class="modal-dialog modal-small"><div class="modal-content"><div class="modal-header"><h4 class="modal-title">'+currentConfig.dialogTitle+'</h4></div><div class="modal-body"><p>'+currentConfig.dialogText+' <span id="idle-timeout-counter"></span>.</p><p>Do you want to continue your session?</p></div><div class="modal-footer"><button id="idle-timeout-dialog-logout" type="button" class="btn btn-default">'+currentConfig.dialogLogOutNowButton+'</button><button id="idle-timeout-dialog-keepalive" type="button" class="btn btn-primary" data-dismiss="modal">'+currentConfig.dialogStayLoggedInButton+'</button></div></div></div></div>');

			$('#idle-timeout-dialog').modal('show');
			$('#idle-timeout-dialog-logout').click(function(){
				logoutUser();
			});
			$('#idle-timeout-dialog-keepalive').click(function(){
				destroyWarningDialog();
				stopDialogTimer();
				startIdleTimer();
			});
		...
		isDialogOpen = function () {
			var dialogOpen = $("#idle-timeout-dialog").is(":visible");
		...
		countdownDisplay = function () {
			...
			remainingTimer = setInterval(function () {
				...
				$('#idle-timeout-counter').html(mins + ':' + secs);
